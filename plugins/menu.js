...
const handler = async (m, { conn, usedPrefix: _p, command, isOwner, args }) => { const allTags = {
...
  // Safe display name for the bot — conn.user.name can briefly be
  // undefined right after connect/reconnect, before the push-name syncs.
  const botName = (conn.user && conn.user.name) global.namebot 'Bot'

  const defaultMenu = {
   before: 
╭━━━⪩ ${toBoldUnicode(botName)} ⪨━━━⬣
┃ 👋 ${ucapan()}, %name
...
  // Rows now respect the same owner/group visibility rules as the menu itself
  const visibleTagKeys = Object.keys(allTags).filter(tag => {
   if (tag === 'owner' && !isOwner) return false
   if (tag === 'group' && !m.isGroup) return false
   return true
  })

  const rows = [
   ...visibleTagKeys.map(tag => {
    const count = help.filter(p => p.tags.includes(tag)).reduce((a, p) => a + p.help.length, 0)    return {
...
  // Sort keys longest-first so %prefix isn't swallowed by the shorter %p, etc.
  const replaceKeys = Object.keys(replace).sort((a, b) => b.length - a.length)  await conn.sendButton(
   m.chat,
   {
    image: { url: 'https://files.catbox.moe/vyr24s.png' },
    caption: text.replace(
     new RegExp(%(${replaceKeys.join('|')}), 'g'),
     (_, key) => replace[key]
    ),
