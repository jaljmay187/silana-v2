...
const handler = async (m, { conn, usedPrefix: _p, command, isOwner, args }) => {
 ...
 try {
  ...
  // --- defensive guards: make sure the shared state actually exists ---
  // on a fresh Railway deploy the db file can start empty/undefined,
  // which used to crash this whole handler before it could send anything.
  ...
  const defaultMenu = {
   before: 
╭━━━⪩ ${toBoldUnicode(conn.user?.name || global.namebot || 'Bot')} ⪨━━━⬣
┃ 👋 ${ucapan()}, %name
...
%readmore.trim(),
  ...
  // conn.getName may not exist depending on the framework build — fall back safely
  let name = registered && user.name
   ? user.name
   : (typeof conn.getName === 'function' ? conn.getName(m.sender) : (m.pushName || 'User'))
  ...
  let rtotalreg = Object.values(global.db.data.users).filter(u => u && u.registered).length
  ...
  const replace = {
   ...
   prefix: _p,
   uptime,
   me: conn.user?.name  global.namebot  'Bot',
   name,
   week,
   date,
   totalreg,
  ...
  const finalCaption = text.replace(
   new RegExp(%(${Object.keys(replace).join('|')}), 'g'),
   (_, key) => replace[key]
  )

  // conn.sendButton is a custom extension some forks add to the socket —
  // if it isn't present on this build, fall back to a plain image+caption
  // message instead of crashing the whole menu.
  if (typeof conn.sendButton === 'function') {
   await conn.sendButton(
    m.chat,
    {
     ...
     footer: ${global.namebot || 'Bot'} • ${new Date().toLocaleTimeString('en-US')},
     buttons: [
      {
       name: 'single_select',
   ...
   await conn.sendMessage(
    m.chat,
    { image: { url: 'https://files.catbox.moe/vyr24s.png' }, caption: finalCaption },
    { quoted: m }
   )
  }
  ...
  await m.react('❌')
  // show the real error instead of swallowing it, so it's fixable from WhatsApp too
